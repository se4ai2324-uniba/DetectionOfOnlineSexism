...
class DetectionOfOnlineSexismUser(HttpUser):
    ...
    def get_metrics_task_A(self):
        ...
        if not all(key in metrics for key in expected_keys):
            raise Exception(f"Metrics response for task A is incomplete")
    ...
    def task_A_preprocessing(self):
        ...
        if not all(key in response_json for key in expected_keys):
            raise Exception(f"The preprocessing description response for task A is incomplete")
    ...
    def task_B_preprocessing(self):
        ...
        expected_keys = ["title", "tokenizer", "vectorizer", "lemmatizer"]
        if not all(key in response_json for key in expected_keys):
            raise Exception(f"The preprocessing description response for task B is incomplete")

    @task(3)
    def prediction_sexism(self):
        message_sexism = [{"message": "When a girl gives you shit test, scare the shit out of her."},
        ...
        if response.status_code != 200:
            input = message_sexism["message"]
            raise Exception(f"Prediction failed for sentence: {input}")
        if "prediction" not in response.json():
            raise Exception(f"No mask in the prediction response for sentence: {input}")

    @task(3)
    def prediction_category(self):
        message_category = [{"message": "Fuck Claire McCaskill, she is a super twat. I am voting against her, and everyone else in Missouri should too."},
        ...
        if response.status_code != 200:
            input = message_category["message"]
            raise Exception(f"Prediction failed for sentence: {input}")
        if "prediction" not in response.json():
            raise Exception(f"No mask in the prediction response for sentence: {input}")
